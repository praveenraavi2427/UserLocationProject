...
class Users(Resource):
    def get(self):
        data = pd.read_csv('users_account.csv')  # read local CSV
    ...
    def post(self):
        ...
        # read our CSV
        data = pd.read_csv('users_account.csv')
    ...
    def put(self):
        ...
        if args['userId'] in list(data['userId']):
            ...
            # save back to CSV
            data.to_csv('users_account.csv', index=False)
...
class Locations(Resource):
    def get(self):
        data = pd.read_csv('locations_bangalore.csv')  # read local CSV
    ...
    def post(self):
        ...
        if args['locationId'] in list(data['locationId']):
            ...
            # add the newly provided values
            data = data.append(new_data, ignore_index=True)
            data.to_csv('locations_bangalore.csv', index=False)  # save back to CSV
            return {'data': data.to_dict()}, 200  # return data with 200 OK
